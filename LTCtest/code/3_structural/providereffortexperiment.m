function solutionSNF=providereffortexperiment(alpha, beta, kappa, mc, costtau, phi, ...
    Theta, delta, mu, rho, psi, revenue, price, utility, dischShock_cons, dinc)
    ...
    while metric > tol
        ...
        valueSnfPrv = (profit(1) - effort.^costtau + ... %flow payoff
            ...
            valueSnfMcdOld) * max(1 - probDisch(1) - alpha * effort, 0) + ... % no discharge
            delta * Theta * ((phi .* (rho * valueSnfPrvOld + (1 - rho) * ...
            valueSnfMcdOld)) + (1-phi) .* valueSnfNonOld) * ...
        ...
        valueSnfMcd = (profit(2) - effort.^costtau + ... %flow payoff
            ...
            delta*min(probDisch(2)-mu(2)+ alpha * effort,1)*dinc ; % discharge; % discharge
