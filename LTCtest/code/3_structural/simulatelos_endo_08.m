...
for t=1:T
 ...
 [~, occu_markerpost] = min(abs(occup_endo_PartCdf(occu_marker, :) - simu_shock2(:,t)),[],2);
 for i=1:Numsim
 if occup_endo_PartCdf(occu_marker(i),occu_markerpost(i)) - simu_shock2(i,t)<0 & occu_markerpost(i)<25
...
for t=1:T
 ...
 %[~, occu_marker] = min(abs(occupTransPartCdf(occu_marker, :) - simu_shock4(:,t)),[],2);
  [~, occu_markerpost] = min(abs(occup_endo_PartCdf(occu_marker, :) - simu_shock4(:,t)),[],2);
 for i=1:Numsim
 if occup_endo_PartCdf(occu_marker(i),occu_markerpost(i)) - simu_shock4(i,t)<0 & occu_markerpost(i)<25
     occu_markerpost(i)=occu_markerpost(i)+1;
 end
 end
 occu_med = occupPart(occu_markerpost); %%% occupancy at beginning of period
